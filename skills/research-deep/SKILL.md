...
**Agent Reach** は X/Twitter・YouTube・Reddit・GitHub・TikTok など18+プラットフォームをAPI料金なしで閲覧・検索できるOSS CLI。インストール済みの場合、各アイテムの深掘り調査にソーシャルメディア情報を追加できる。

### 各アイテムのリサーチプロンプトへの追記（Agent Reach 使用時）

Step 3 のプロンプトテンプレートに以下を追加する：

```python
agent_reach_section = """
## Agent Reach（インストール済みの場合）
以下のコマンドで追加情報を取得し、JSONのフィールドを補完する:

# YouTube: 関連動画・レビュー・チュートリアルを検索
agent-reach youtube search "{item_name}" --limit 3

# Reddit: コミュニティの評判・議論を取得
agent-reach reddit search "{item_name}" --limit 5

# GitHub: リポジトリのスター数・Issue・最終更新を確認
agent-reach github search "{item_name}" --type repos --sort stars --limit 3

取得できた情報はJSONの該当フィールドに反映し、ソースURLをsourcesフィールドに追加する。
Agent Reachがインストールされていない場合はこのステップをスキップしてWebSearchのみで続行する。
"""
```
...
### 注意事項
- Agent Reach はオプション。未インストールの場合はWebSearchのみで続行する
